...
//分页的索引
var pageIndex = 1;
var isOnlyOnce = true; //用来控制picker组件，只在初始渲染数据时，渲染一次
//是够清除 列表的数据
var clearFlag = true;
//用来保存当前的维度，来确定层级的顺序
...
$(function() {
    //cordova  初始化并获取数据
    document.addEventListener('deviceready', initWebApp, false);
    //发送请求，调取数据
    //点击箭头返回
    ...
    $(".navWraper").on("click", "li", function() {
        ...
        //获取用户的信息
    function initWebApp() {
        cordova.exec(callbacksucce, callbackerr, "WebAppPlugin", "initWebApp", [])

        function callbacksucce(data) {
            try {
                data = JSON.parse(data)
            } catch (e) {
                data = data
            }
            cuserid = data.data.cuserid;
            callActionData({
        ...
        function callbackerr(data) {
            mui.alert("获取用户信息失败！", "提示", "确定", function() {})
        }
    }
            ...
            $.dateSelector({
                ...
                afterAction: function(d1, d2) {
                    ...
                    if (d2.indexOf("月") >= 0) {
                        ...
                        wholeVariateTime.begindate = d1 + "/" + $.formatMonth(checkMonth) + "/01"
                        wholeVariateTime.enddate = d1 + "/" + $.formatMonth(checkMonth) + "/" + checkDays
                        ...
                        wholeVariateTime.begindate = d1 + "/" + $.formatMonth(checkFirstMonth) + "/01"
                        wholeVariateTime.enddate = d1 + "/" + $.formatMonth(checkEndtMonth) + "/" + checkDays
                    } else {
                        wholeVariateTime = {
                            begindate: new Date().getFullYear() + '/01/01',
                            enddate: new Date().getFullYear() + '/12/31',
    ...
    function drawEhart(data) {
        ...
        data.forEach(function(item, index) {
            ...
            var randNum = $.randomNum(8) //保证相邻的俩个数不相等
    ...
    function listBindEvent() {
        ...
        mui("#list-wraper").on("tap", "li", function() {
            ...
            params.begindate = wholeVariateTime.begindate;
            params.enddate = wholeVariateTime.enddate;
            ...
            function mycallback(data) {
                ...
                if (data.datas === null) {
                    mui.toast('没有更多的数据！', { duration: 'short', type: 'div' })
                    return;
                }
                $("#checkTimeRang").hide();
                currentIndex++;
    ...
    function callActionData(options) {
        var params = {};
        params.begindate = options.startTime || new Date().getFullYear() + '/01/01';
        params.enddate = options.endTime || new Date().getFullYear() + '/12/31';
        params.cuserid = cuserid;
